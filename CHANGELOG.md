...
### Fixed
- fix: 싱크룸 크루곡 누수 차단 + 구간 변경 시 로컬 검증 무효화 (a0453ed, #76)
  - 크루 신호가 **섹션 헤더에만** 있는 합방을 파서가 못 걸렀다(`🎵 하데스 후열 싱크룸 - 솜키띵초챈 순서`).
    각 곡 줄에 소속 헤더를 붙여 판정하도록 바꿨고, 즉석 조합 아티스트(`솜띵키초`)는 멤버 음절 집합의
    부분집합인지로 걸러낸다. 73개 VOD 전수 역검증에서 파싱 곡수 488 → 456, 감소분 32곡은 전부 크루곡이며
    현재 DB 기록 영향 0.
  - `local_review=verified`인 performance의 구간을 바꿔도 검증이 남아 재검증 없이 유튜브 게이트를
    통과하던 문제. `update_performance`가 구간 이동을 감지하면 `pending`으로 되돌린다.
