...
### Fixed
- fix(identify): 신곡 draft 등록 전 가사로 카탈로그 역조회 (ffe0772, #78)
  - `resolve_song_match`는 가사를 **제목 기반 shortlist 안에서만** 타이브레이커로 써서, 제목 추측이
    통째로 틀리면 정답이 후보에 들지 못하고 카탈로그에 있는 곡이 신곡 draft로 등록된다
    (perf #1370: 실제는 `Mela!`인데 `メランコリック` draft가 생겨 잘못된 제목이 유튜브 오버레이로
    구워진 채 업로드됨). 제목을 거치지 않고 **가사 대 가사**로 카탈로그를 훑는 경로를 추가하고,
    `add-song`이 삽입 전에 검사해 닮은 곡이 있으면 후보를 보여주며 거절한다(`--force`로 통과).
  - 임계값 45는 실측 근거: 정답 `Mela!` 61.9점 대 2위 26.3점, 기존 draft 59개 소급 점검에서
    8건(14%) 검출·그중 7건이 실제 중복. 이 점검으로 라이브 performance 6개를 기존 곡으로 재연결했다.
