...
- **마커 의미(댓글 기준):** `🎤`=BJ 솔로(대상). `🎵`/`🎶`=게스트/크루/콘서트/클립(대상 아님).
  ...
  - **예외 — 타임라인이 통째로 🎵인 VOD.** 팬마다 표기가 달라, 잡담 줄 없이 **노래만 나열한**
    타임라인은 곡 전부를 🎵로 적기도 한다. 이때 아티스트 슬롯이 전부 **원곡 가수**면 BJ 솔로
    노래뱅이다(🎤 0개라 `manual`로 빠진다 — `ingest.md` 2번). 아티스트 슬롯에 **다른 스트리머
    이름**이 있으면 평소대로 게스트/노래배틀이라 대상이 아니다.
...
- **모든 판정에 근거를 붙여라** — 왜 노래/게임/티저/크루로 봤는지 댓글 인용 + 전사 **초 단위 시각**으로. 가사 소절은 인용하지 않는다(출력 필터에 막혀 작업이 끊긴다 — perf.md 규칙 ⚠️).
