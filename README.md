...
> Claude Code에서는 이 전체 흐름(전사 → 솔로 풀곡 선별 → ingest)을 `/manual-ingest [title_no]`
> 스킬로 오케스트레이션한다. I/O는 코드가, "어디부터 어디까지가 BJ 솔로 풀곡인가" 판정은 Claude가 한다.

### 처리 결과 검증 (vod-audit 스킬)

이미 처리된 VOD의 분류가 맞는지 **Claude가 원본 댓글을 직접 읽어** 검증한다. 코드가
"게임 타임라인 vs 노래 타임라인", "티저 언급 vs 실제 부름"을 뭉개는 걸 Claude 판단으로
대체하는 로컬 감사다. Claude Code에서 `/vod-audit`로 호출한다.
