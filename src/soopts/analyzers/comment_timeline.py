...
사람마다 달라 정규식으로 안정적으로 파싱할 수 없다. Gemini에게 댓글 전체를 주고
"노래를 불렀다"고 볼 수 있는 항목만 골라 JSON으로 뽑아낸다(한국어 구어체·이모지가
섞인 댓글이 많아 다국어 처리가 좋은 Gemini를 선택했다).
...
def parse_timeline_json(text: str) -> list[TimelineSong]:
    """Gemini 응답(JSON 배열, 코드펜스 있을 수 있음) → TimelineSong 리스트. 순수 함수."""
...
def extract_song_timeline(comments: list[str], *, api_key: str | None = None) -> list[TimelineSong]:
    ...
    resp = client.models.generate_content(
        ...
        config=types.GenerateContentConfig(
            max_output_tokens=1000, response_mime_type="application/json"
        ),
    )
    songs = parse_timeline_json(resp.text)
    if songs:
