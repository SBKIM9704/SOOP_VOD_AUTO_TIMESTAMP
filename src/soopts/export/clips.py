"""노래 구간 탐지 — 후보 구간에서 노래의 정밀한 시작/끝 시각을 찾는다.

흐름(구간당): 후보 구간을 다운로드 → inaSpeechSegmenter로 음악 경계 정밀 탐지
→ **구간 내 가장 긴 음악 블록 = 노래**로 잡아 그 경계를 시각으로 반환.
실측: 매직카펫 구간에서 정답 대비 1초 오차로 경계 탐지됨.

산출물은 시각이지 파일이 아니다. 예전엔 여기서 ffmpeg로 1080p 클립을 재인코딩해
유튜브에 올렸으나, 그 경로를 제거하면서 재인코딩도 함께 없앴다(구간당 6.6분 →
0초, 실행 시간의 76%를 차지하던 작업). 시청은 SOOP 원본 딥링크로 연결한다.
다운로드한 구간 영상은 경계 탐지와 가사 전사를 위한 임시 입력일 뿐이다.
...
def detect_song_span(
    cfg: Config, media_path: str, region_start: float, region_end: float,
    media_offset: float = 0.0,
) -> tuple[Clip, float, float] | None:
    """구간 영상에서 노래의 정밀 경계를 찾아 (Clip, 로컬시작, 로컬끝)을 반환한다.

    media_offset: media_path의 t=0 이 전역 몇 초인지(슬라이스면 region_start). 경계는 media 로컬
    기준으로 탐지하고, 전역 시각은 offset을 더해 기록한다.

    로컬 경계도 함께 돌려주는 이유: 호출부가 같은 media_path에서 그 구간의 오디오만
    잘라 STT에 넘겨야 하는데, 전역 시각으로는 파일 안 위치를 알 수 없다.
    """
    ...
    log.info("노래 구간 %s~%s (%d초)", fmt_hms(g_start), fmt_hms(g_end), g_end - g_start)
    return Clip(t=g_start, end=g_end, duration=g_end - g_start), ls, le
