...
def test_fmt_chapter_time_drops_hour_when_zero():
    assert fmt_chapter_time(0) == "0:00"
    assert fmt_chapter_time(252) == "4:12"
    assert fmt_chapter_time(3723) == "1:02:03"
...
def test_description_chapter_block_starts_at_zero_and_has_bare_lines():
    ...
    first = next(line for line in lines if line.startswith("0:00"))
    assert first == "0:00 벽지무늬 - 아이유"
    assert lines[lines.index(first) + 1] == "3:23 묘해, 너와 - 어쿠스틱콜라보"
